# PDF Form Builder — turning flat/non-fillable PDFs into real AcroForms
...
### 3. Surgical Semantic Review & Polish (MANDATORY — NEVER SKIP)

Because form authors employ wild and irregular layout/drawing conventions, **auto-generation is a draft, not the final product**. You **must** visually inspect the rendered PNGs and compare them with `field_spec.json`. 

Review and adjust the spec manually to:
- **Prune visual divider lines** that the script incorrectly identified as text entry underlines (which will sit on top of or overlap major section titles or description text).
- **Fine-tune heights/widths** so fields align seamlessly above their corresponding underlines or perfectly fill their bounding input boxes.
- **Set "comb" formats**: If a text entry area consists of individual tick-boxes (e.g., V-number, Postcode, Date), change the `"kind": "text"` to `"kind": "comb"` and set the `"maxlen": N` where N is the number of tick-boxes.
